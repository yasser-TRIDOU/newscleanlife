<div class="front_page_section front_page_section_team<?php
			$gracioza_scheme = gracioza_get_theme_option('front_page_team_scheme');
			if (!gracioza_is_inherit($gracioza_scheme)) echo ' scheme_'.esc_attr($gracioza_scheme);
			echo ' front_page_section_paddings_'.esc_attr(gracioza_get_theme_option('front_page_team_paddings'));
		?>"<?php
		$gracioza_css = '';
		$gracioza_bg_image = gracioza_get_theme_option('front_page_team_bg_image');
...
?>><?php
	// Add anchor
	$gracioza_anchor_icon = gracioza_get_theme_option('front_page_team_anchor_icon');	
	$gracioza_anchor_text = gracioza_get_theme_option('front_page_team_anchor_text');	
	...
	<div class="front_page_section_inner front_page_section_team_inner<?php
			if (gracioza_get_theme_option('front_page_team_fullheight'))
				echo ' gracioza-full-height sc_layouts_flex sc_layouts_columns_middle';
			?>"<?php
			$gracioza_css = '';
			$gracioza_bg_mask = gracioza_get_theme_option('front_page_team_bg_mask');
			$gracioza_bg_color = gracioza_get_theme_option('front_page_team_bg_color');
	...
	?>>
		<div class="front_page_section_content_wrap front_page_section_team_content_wrap content_wrap">
			<?php
			// Caption
			$gracioza_caption = gracioza_get_theme_option('front_page_team_caption');
			if (!empty($gracioza_caption) || (current_user_can('edit_theme_options') && is_customize_preview())) {
				?><h2 class="front_page_section_caption front_page_section_team_caption front_page_block_<?php echo !empty($gracioza_caption) ? 'filled' : 'empty'; ?>"><?php echo wp_kses($gracioza_caption, 'gracioza_kses_content'); ?></h2><?php
			}
		
			// Description (text)
			$gracioza_description = gracioza_get_theme_option('front_page_team_description');
			if (!empty($gracioza_description) || (current_user_can('edit_theme_options') && is_customize_preview())) {
				?><div class="front_page_section_description front_page_section_team_description front_page_block_<?php echo !empty($gracioza_description) ? 'filled' : 'empty'; ?>"><?php echo wp_kses(wpautop($gracioza_description), 'gracioza_kses_content'); ?></div><?php
			}
			...
			?><div class="front_page_section_output front_page_section_team_output"><?php 
				if (is_active_sidebar('front_page_team_widgets')) {
					...
					if (!gracioza_exists_trx_addons())
						...
						gracioza_customizer_need_widgets_message('front_page_team_caption', 'ThemeREX Addons - Team');
				}
			?></div>
		</div>
	</div>
