...
------------------------------------------------------------------------------- */
if (!function_exists('gracioza_trx_addons_theme_specific_setup1')) {
	add_filter( 'trx_addons_filter_components_editor', 'gracioza_trx_addons_theme_specific_components');
	function gracioza_trx_addons_theme_specific_components($enable=false) {
		return false;
	}
}

if (!function_exists('gracioza_trx_addons_theme_specific_setup1')) {
	add_action( 'after_setup_theme', 'gracioza_trx_addons_theme_specific_setup1', 1 );
	add_action( 'trx_addons_action_save_options', 'gracioza_trx_addons_theme_specific_setup1', 1 );
	function gracioza_trx_addons_theme_specific_setup1() {
		if (gracioza_exists_trx_addons()) {
			add_filter( 'trx_addons_cv_enable',					'gracioza_trx_addons_cv_enable');
			add_filter( 'trx_addons_demo_enable',				'gracioza_trx_addons_demo_enable');
			add_filter( 'trx_addons_filter_edd_themes_market',	'gracioza_trx_addons_edd_themes_market_enable');
			add_filter( 'trx_addons_cpt_list',					'gracioza_trx_addons_cpt_list');
			add_filter( 'trx_addons_sc_list',					'gracioza_trx_addons_sc_list');
			add_filter( 'trx_addons_widgets_list',				'gracioza_trx_addons_widgets_list');
		}
	}
}

// CV
if ( !function_exists( 'gracioza_trx_addons_cv_enable' ) ) {
	
	function gracioza_trx_addons_cv_enable($enable=false) {
		// To do: return false if theme not use CV functionality
		return GRACIOZA_THEME_FREE
					? false		// Free version
					: false;		// Pro version
	}
}
...
// Demo mode
if ( !function_exists( 'gracioza_trx_addons_demo_enable' ) ) {
	
	function gracioza_trx_addons_demo_enable($enable=false) {
...
// EDD Themes market
if ( !function_exists( 'gracioza_trx_addons_edd_themes_market_enable' ) ) {
	
	function gracioza_trx_addons_edd_themes_market_enable($enable=false) {
...
// API
if ( !function_exists( 'gracioza_trx_addons_api_list' ) ) {
	
	function gracioza_trx_addons_api_list($list=array()) {
...
// CPT
if ( !function_exists( 'gracioza_trx_addons_cpt_list' ) ) {
	
	function gracioza_trx_addons_cpt_list($list=array()) {
...
// Shortcodes
if ( !function_exists( 'gracioza_trx_addons_sc_list' ) ) {
	
	function gracioza_trx_addons_sc_list($list=array()) {
...
// Widgets
if ( !function_exists( 'gracioza_trx_addons_widgets_list' ) ) {
	
	function gracioza_trx_addons_widgets_list($list=array()) {
...
if ( !function_exists( 'gracioza_trx_addons_menu_cache' ) ) {
	add_filter( 'trx_addons_filter_menu_cache', 'gracioza_trx_addons_menu_cache');
	function gracioza_trx_addons_menu_cache($list=array()) {
...
if (!function_exists('gracioza_trx_addons_localize_script')) {
	...
	function gracioza_trx_addons_localize_script($arr) {
		$arr['alter_link_color'] = gracioza_get_scheme_color('alter_link');
		return $arr;
	}
}
...
if ( !function_exists( 'gracioza_trx_addons_sc_type' ) ) {
	add_filter( 'trx_addons_sc_type', 'gracioza_trx_addons_sc_type', 10, 2);
	function gracioza_trx_addons_sc_type($list, $sc) {
...
if ( !function_exists( 'gracioza_trx_addons_sc_atts' ) ) {
	add_filter( 'trx_addons_sc_atts', 'gracioza_trx_addons_sc_atts', 10, 2);
	function gracioza_trx_addons_sc_atts($atts, $sc) {
...
if ( !function_exists( 'gracioza_trx_addons_sc_map' ) ) {
	add_filter( 'trx_addons_sc_map', 'gracioza_trx_addons_sc_map', 10, 2);
	function gracioza_trx_addons_sc_map($params, $sc) {
		...
		$param = array(
			...
			'heading' => esc_html__('Color style', 'gracioza'),
			'description' => wp_kses_data( __('Select color style to decorate this block', 'gracioza') ),
			'edit_field_class' => 'vc_col-sm-4',
			'admin_label' => true,
			'value' => array_flip(gracioza_get_list_sc_color_styles()),
			'type' => 'dropdown'
		);
...
if ( !function_exists( 'gracioza_trx_addons_sow_map' ) ) {
	add_filter( 'trx_addons_sow_map', 'gracioza_trx_addons_sow_map', 10, 2);
	function gracioza_trx_addons_sow_map($params, $sc) {

		// Param 'color_style'
		$param = array(
			'color_style' => array(
				'label' => esc_html__('Color style', 'gracioza'),
				'description' => wp_kses_data( __('Select color style to decorate this block', 'gracioza') ),
				'options' => gracioza_get_list_sc_color_styles(),
				'default' => 'default',
				'type' => 'select'
			)
		);
		if (in_array($sc, array('trx_sc_button')))
			gracioza_array_insert_after($params, 'size', $param);
		else if (in_array($sc, array('trx_sc_action', 'trx_sc_blogger', 'trx_sc_cars', 'trx_sc_courses', 'trx_sc_content', 'trx_sc_dishes',
								'trx_sc_events', 'trx_sc_form',	'trx_sc_googlemap', 'trx_sc_portfolio', 'trx_sc_price', 'trx_sc_promo',
								'trx_sc_properties', 'trx_sc_services', 'trx_sc_team', 'trx_sc_testimonials', 'trx_sc_title',
								'trx_widget_audio', 'trx_widget_twitter')))
			gracioza_array_insert_after($params, 'title_align', $param);
		return $params;
	}
}

// Add classes to the shortcode's output
if ( !function_exists( 'gracioza_trx_addons_sc_output' ) ) {
	add_filter( 'trx_addons_sc_output', 'gracioza_trx_addons_sc_output', 10, 4);
	function gracioza_trx_addons_sc_output($output, $sc, $atts, $content) {
		...
		if (in_array($sc, array('trx_sc_action'))) {
			...
			if (!empty($atts['color_style']) && !gracioza_is_inherit($atts['color_style']))
...
if ( !function_exists( 'gracioza_trx_addons_sc_item_title_tag' ) ) {
	add_filter( 'trx_addons_filter_sc_item_title_tag', 'gracioza_trx_addons_sc_item_title_tag');
	function gracioza_trx_addons_sc_item_title_tag($tag='') {
		return $tag=='h1' ? 'h2' : $tag;
	}
}

// Return args for the item's button
if ( !function_exists( 'gracioza_trx_addons_sc_item_button_args' ) ) {
	add_filter( 'trx_addons_filter_sc_item_button_args', 'gracioza_trx_addons_sc_item_button_args', 10, 3);
	function gracioza_trx_addons_sc_item_button_args($args, $sc, $sc_args) {
...
if ( !function_exists( 'gracioza_trx_addons_slider_title' ) ) {
	add_filter( 'trx_addons_filter_slider_title',	'gracioza_trx_addons_slider_title', 10, 2 );
	function gracioza_trx_addons_slider_title($title, $data) {
		$title = '';
		if (!empty($data['title'])) 
...
if ( !function_exists( 'gracioza_trx_addons_sc_googlemap_styles' ) ) {
	add_filter( 'trx_addons_filter_sc_googlemap_styles',	'gracioza_trx_addons_sc_googlemap_styles');
	function gracioza_trx_addons_sc_googlemap_styles($list) {
		$list['dark'] = esc_html__('Dark', 'gracioza');
		return $list;
	}
}
...
if ( !function_exists( 'gracioza_trx_addons_tiny_mce_style_formats' ) ) {
	add_filter( 'trx_addons_filter_tiny_mce_style_formats', 'gracioza_trx_addons_tiny_mce_style_formats');
	function gracioza_trx_addons_tiny_mce_style_formats($style_formats) {
		...
		if (is_array($style_formats) && count($style_formats)>0 ) {
			foreach ($style_formats as $k=>$v) {
                if ( $v['title'] == esc_html__('Inline', 'gracioza') ) {
                    $style_formats[$k]['items'][] = array(
                        'title' => esc_html__('Widget title', 'gracioza'),
                        'inline' => 'span',
                        'classes' => 'trx_addons_widget_title'
                    );
                }
			}
		}
...
if ( !function_exists( 'gracioza_trx_addons_allow_override_header_image' ) ) {
	add_filter( 'gracioza_filter_allow_override_header_image', 'gracioza_trx_addons_allow_override_header_image' );
	function gracioza_trx_addons_allow_override_header_image($allow) {
		return gracioza_is_team_page() ? false : $allow;
	}
}
...
if (!function_exists('gracioza_trx_addons_override_options_fields')) {
	add_filter( 'trx_addons_filter_override_options_fields', 'gracioza_trx_addons_override_options_fields', 10, 2);
	function gracioza_trx_addons_override_options_fields($mb, $post_type) {
		if (defined('TRX_ADDONS_CPT_TEAM_PT') && $post_type==TRX_ADDONS_CPT_TEAM_PT) {
			$mb['email'] = array(
				"title" => esc_html__("E-mail",  'gracioza'),
				"desc" => wp_kses_data( __("Team member's email", 'gracioza') ),
				"std" => "",
				"details" => true,
				"type" => "text"
			);
        ...
        if (defined('TRX_ADDONS_CPT_TESTIMONIALS_PT') && $post_type==TRX_ADDONS_CPT_TESTIMONIALS_PT) {
            $mb['years'] = array(
                "title" => esc_html__("Age",  'gracioza'),
                "desc" => wp_kses_data( __("Team member's age", 'gracioza') ),
                "std" => "",
                "details" => true,
                "type" => "text"
            );
        ...
        if (defined('TRX_ADDONS_CPT_SERVICES_PT') && $post_type==TRX_ADDONS_CPT_SERVICES_PT) {
            $mb['services-type'] = array(
                "title" => esc_html__("Select type for services",  'gracioza'),
                "desc" => wp_kses_data( __("Select type for services in classic blog style", 'gracioza') ),
                "std" => "",
                "options" => array(
                    "normal" => esc_html__("Normal", 'gracioza'),
                    "style-1" => esc_html__("Style 1", 'gracioza'),
                    "style-2" => esc_html__("Style 2", 'gracioza')
                ),
                "type" => "select2"
            );
            $mb['banner'] = array(
                "title" => esc_html__("Banner text",  'gracioza'),
                "desc" => wp_kses_data( __("Text for banner in classic blog style", 'gracioza') ),
                "dependency" => array(
                    'services-type' => array('style-1', 'style-2')
                ),
                "std" => "",
                "type" => "text"
            );
