...
if (!function_exists('gracioza_hovers_theme_setup3')) {
	add_action( 'after_setup_theme', 'gracioza_hovers_theme_setup3', 3 );
	function gracioza_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		gracioza_storage_set_array_after('options', 'border_radius', array(
			'button_hover' => array(
				"title" => esc_html__("Button's hover", 'gracioza'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme buttons', 'gracioza') ),
				"std" => 'default',
				"options" => array(
					'default'		=> esc_html__('Fade',				'gracioza'),
					'slide_left'	=> esc_html__('Slide from Left',	'gracioza'),
					'slide_right'	=> esc_html__('Slide from Right',	'gracioza'),
					'slide_top'		=> esc_html__('Slide from Top',		'gracioza'),
					'slide_bottom'	=> esc_html__('Slide from Bottom',	'gracioza'),
				),
				"type" => "select"
			),
			'image_hover' => array(
				"title" => esc_html__("Image's hover", 'gracioza'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme images', 'gracioza') ),
				"std" => 'dots',
				"override" => array(
					'mode' => 'page',
				...
				"options" => array(
					'dots'	=> esc_html__('Dots',	'gracioza'),
					'icon'	=> esc_html__('Icon',	'gracioza'),
					'icons'	=> esc_html__('Icons',	'gracioza'),
					'zoom'	=> esc_html__('Zoom',	'gracioza'),
					'fade'	=> esc_html__('Fade',	'gracioza'),
					'slide'	=> esc_html__('Slide',	'gracioza'),
					'pull'	=> esc_html__('Pull',	'gracioza'),
					'border'=> esc_html__('Border',	'gracioza')
				),
				"type" => "select"
			) )
		);
...
if (!function_exists('gracioza_hovers_theme_setup9')) {
	add_action( 'after_setup_theme', 'gracioza_hovers_theme_setup9', 9 );
	function gracioza_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts',		'gracioza_hovers_frontend_scripts', 1010 );
		add_filter( 'gracioza_filter_localize_script','gracioza_hovers_localize_script' );
		add_filter( 'gracioza_filter_merge_scripts',	'gracioza_hovers_merge_scripts' );
		add_filter( 'gracioza_filter_merge_styles',	'gracioza_hovers_merge_styles' );
		add_filter( 'gracioza_filter_get_css', 		'gracioza_hovers_get_css', 10, 4 );
	}
}
	
// Enqueue hover styles and scripts
if ( !function_exists( 'gracioza_hovers_frontend_scripts' ) ) {
	
	function gracioza_hovers_frontend_scripts() {
		if ( gracioza_is_on(gracioza_get_theme_option('debug_mode')) && gracioza_get_file_dir('theme-specific/theme.hovers/theme.hovers.js')!='' )
			wp_enqueue_script( 'gracioza-hovers', gracioza_get_file_url('theme-specific/theme.hovers/theme.hovers.js'), array('jquery'), null, true );
		if ( gracioza_is_on(gracioza_get_theme_option('debug_mode')) && gracioza_get_file_dir('theme-specific/theme.hovers/theme.hovers.css')!='' )
			wp_enqueue_style( 'gracioza-hovers',  gracioza_get_file_url('theme-specific/theme.hovers/theme.hovers.css'), array(), null );
	}
}

// Merge hover effects into single js
if (!function_exists('gracioza_hovers_merge_scripts')) {
	
	function gracioza_hovers_merge_scripts($list) {
...
if (!function_exists('gracioza_hovers_merge_styles')) {
	
	function gracioza_hovers_merge_styles($list) {
...
if (!function_exists('gracioza_hovers_localize_script')) {
	
	function gracioza_hovers_localize_script($arr) {
		$arr['button_hover'] = gracioza_get_theme_option('button_hover');
		return $arr;
	}
}

// Add hover icons on the featured image
if ( !function_exists('gracioza_hovers_add_icons') ) {
	function gracioza_hovers_add_icons($hover, $args=array()) {
		...
		// Hover style 'Shop'
			...
			<div class="icons">
				...
				if (!is_object($args['cat']) && $product->is_purchasable() && $product->is_in_stock()) {
					gracioza_show_layout(apply_filters( 'woocommerce_loop_add_to_cart_link',
										'<a rel="nofollow" href="' . esc_url($product->add_to_cart_url()) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ($hover == 'shop_buttons' ? esc_html__('Buy now', 'gracioza') : '')
										. '</a>',
										$product));
				}
				?><a href="<?php echo esc_url(is_object($args['cat']) ? get_term_link($args['cat']->slug, 'product_cat') : get_permalink()); ?>" aria-hidden="true" class="shop_link button icon-link"><?php
					if ($hover == 'shop_buttons') {
						if (is_object($args['cat']))
							esc_html_e('View products', 'gracioza');
						else
							esc_html_e('Details', 'gracioza');
					}
				?></a>
			</div>
			<?php
...
// Add styles into CSS
if ( !function_exists( 'gracioza_hovers_get_css' ) ) {
	
	function gracioza_hovers_get_css($css, $colors, $fonts, $scheme='') {
