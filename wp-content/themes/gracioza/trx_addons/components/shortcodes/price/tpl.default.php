		...
		?>>
			...
			if (!empty($item['image'])) {
				...
				if (!empty($item['image'])) {
					...
					?><div class="sc_price_item_image"><img src="<?php echo esc_url($item['image']); ?>" alt="<?php esc_attr__('image', 'gracioza')?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>></div><?php
				}
			} else {
				...
				if (!empty($icon)) {
					...
					?><div class="sc_price_item_icon sc_price_item_icon_type_<?php echo esc_attr($item['icon_type']); ?> <?php echo esc_attr($icon); ?>"
						...
						if (!empty($img)) {
							...
							?><img class="sc_icon_as_image" src="<?php echo esc_url($img); ?>" alt="<?php esc_attr__('image', 'gracioza')?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
						} else {
				...
				if (!empty($item['price'])) {
					...
					?><div class="sc_price_item_price"><?php
						if (!empty($item['before_price'])) {
							?><span class="sc_price_item_price_before"><?php echo wp_kses(trx_addons_parse_codes($item['before_price']), 'gracioza_kses_content' ); ?></span><?php
						}
						?><span class="sc_price_item_price_value"><?php echo wp_kses($parts[0], 'gracioza_kses_content' ); ?></span><?php
						if (count($parts) > 1 && $parts[1]!='') {
							?><span class="sc_price_item_price_decimals"><?php echo wp_kses($parts[1], 'gracioza_kses_content' ); ?></span><?php
						}
						if (!empty($item['after_price'])) {
							?><span class="sc_price_item_price_after"><?php echo wp_kses(trx_addons_parse_codes($item['after_price']), 'gracioza_kses_content' ); ?></span><?php
						}
					?></div><?php
				}
