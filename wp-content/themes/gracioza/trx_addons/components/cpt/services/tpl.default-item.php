...
?>>
		...
		if ( has_post_thumbnail() && $args['featured']=='image') {
			trx_addons_get_template_part('templates/tpl.featured.php',
											...
											apply_filters('trx_addons_filter_args_featured', array(
																...
																'hover' => 'zoomin',
																'thumb_size' => apply_filters('trx_addons_filter_thumb_size', trx_addons_get_thumb_size($args['columns'] > 2 ? 'information' : 'information'), 'services-default'),
			...
			?>><?php
				if (!empty($svg)) {
					...
					?><img class="sc_icon_as_image" src="<?php echo esc_url($img); ?>" alt="'.esc_attr__('img', 'gracioza').'"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
				}
			?></a><?php
		} else {
	...
	<div class="sc_services_item_info">
		...
		<?php if (!isset($args['hide_excerpt']) || (int)$args['hide_excerpt']==0) { ?>
			...
			<div class="sc_services_item_button sc_item_button"><a href="<?php echo esc_url($link); ?>" class="<?php echo esc_attr(apply_filters('trx_addons_filter_sc_item_link_classes', 'sc_button sc_button color_style_link2', 'sc_services', $args)); ?>"><?php esc_html_e('More info', 'gracioza'); ?></a></div>
		<?php } ?>
	</div>
</div>
<?php
