...
class Essential_Grid_Base
{
	...
	public static function esg_custom_query_join_table( $join ) {
		...
		$join .= " LEFT JOIN $wpdb->term_relationships esg_tr ON ($wpdb->posts.ID = esg_tr.object_id) ";
		$join .= " LEFT JOIN $wpdb->term_taxonomy esg_tt ON (esg_tr.term_taxonomy_id = esg_tt.term_taxonomy_id) ";
			...
			foreach ($categories as $key => $category) {
				...
				switch ($do_type) {
					...
					default:
						...
						$thelist .= '<a href="' . esc_url($url) . '" title="' . esc_attr(sprintf(esc_attr__('View all posts in %s', ESG_TEXTDOMAIN), $category->name)) . '" ' . $rel . '>' . $category->name . '</a>';
						break;
				}
			}
		}
		...
		if ($content !== null) {
			if (has_shortcode($content, 'gallery')) {
				...
				if (function_exists('parse_blocks')) {
					// KRISZTIAN CHANGE - PLEASE DOUBLE CHECK- FALLBACK
