		...
		if ($new !== false && !empty($new) && is_array($new)) {
			if (empty($cur)) {
				...
				if (isset($new['grids']) && is_array($new['grids'])) {
					...
					foreach ($cur['grids'] as $ck => $c) {
						//remove no longer available grids
						if (!isset($c['exists'])) {
							unset($cur['grids'][$ck]);
						} else {
	...
	public function get_library_grids_html($tp_grids)
	{
		...
		if (!empty($tp_grids)) {
			foreach ($tp_grids as $isd => $grid) {
				...
				$etikett_a = esc_attr__('Premium', ESG_TEXTDOMAIN);

				if ($isnew) {
					$esg_cats[] = 'temp_newupdate';
				}
				?>
				<div class="esg_group_wrappers <?php echo implode(' ', $esg_cats); ?> not-imported-wrapper <?php if (isset($grid['filter'])) {
					echo implode(' ', $grid['filter']);
				} ?>">
					<?php
					...
					<div class="library_thumb_title"><?php echo $grid['title']; ?>
						<?php if ($isnew) { ?>
							<span class="library_new esg_library_filter_button esg_libr_new_udpated"><?php esc_html_e("New", ESG_TEXTDOMAIN); ?></span>
						<?php } ?>
					</div>
				</div>
				<?php
		...
		<div class="library_thumb_more">
			...
			if ($deny == '' && $allow_install == true) {
				...
				<div class="install_library_grid<?php echo $deny; ?>" data-zipname="<?php echo $grid['zip']; ?>"
					 data-uid="<?php echo $grid['uid']; ?>" data-title="<?php echo esc_html($grid['title']); ?>"><i
							class="eg-icon-download"></i><?php esc_html_e('Install Grid', ESG_TEXTDOMAIN); ?></div>
				<?php
			} else {
				?>
				<div class="dontadd_library_grid_item"><i
							class="icon-not-registered"></i><?php esc_html_e('Requirements not met', ESG_TEXTDOMAIN); ?></div>
				<?php
			}
			?>
