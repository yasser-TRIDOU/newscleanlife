			...
			if (!empty($notices) && is_array($notices)) {
				...
				foreach ($notices as $notice) {
					...
					if (!in_array($notice->code, $notices_discarded) && version_compare($notice->version, Essential_Grid::VERSION, '>=')) {
						$text = '<div class="esg-notices-button-container"><a href="javascript:void(0);"  class="esg-notices-button esg-notice-' . esc_attr($notice->code) . '">' . esc_attr__('Close & don\'t show again<b>X</b>', ESG_TEXTDOMAIN) . '</a></div>';
								...
								if (isset($cat[$tax][$c])) { 
									...
									if (!empty($lang)) {
										foreach ($lang as $code => $val) {
											...
											delete_transient('ess_grid_trans_query_' . $grid->id . '_m_' . $val['language_code']);
											delete_transient('ess_grid_trans_full_grid_' . $grid->id . '_m_' . $val['language_code']);
										}
									} else {
										delete_transient('ess_grid_trans_query_' . $grid->id);
										delete_transient('ess_grid_trans_full_grid_' . $grid->id);
										delete_transient('ess_grid_trans_query_' . $grid->id . '_m');
										delete_transient('ess_grid_trans_full_grid_' . $grid->id . '_m');
									}
									$cont = true;
								}
								if ($cont == true) break;
				...
				switch (self::getPostVar("client_action", false)) {
					...
					case 'get_preview_html_markup':
						...
						if (isset($data['id'])) {
							...
							delete_transient('ess_grid_trans_query_' . $data['id'] . '_m_' . $lang_code); //delete cache
						}
							...
							if (isset($data['id']) && intval($data['id']) > 0) {
								if (!empty($lang)) {
									foreach ($lang as $code => $val) {
										...
										delete_transient('ess_grid_trans_query_' . $data['id'] . '_m_' . $val['language_code']); //delete cache
										delete_transient('ess_grid_trans_full_grid_' . $data['id'] . '_m_' . $val['language_code']); //delete cache
									}
								} else {
									...
									delete_transient('ess_grid_trans_query_' . $data['id'] . '_m'); //delete cache
									delete_transient('ess_grid_trans_full_grid_' . $data['id'] . '_m'); //delete cache
								}
