...
if ( !function_exists( 'trx_addons_cpt_layouts_show_layout' ) ) {
	...
	function trx_addons_cpt_layouts_show_layout($id) {
		$layout = get_post($id);
		if (!empty($layout)) {
		    $content = str_replace(array('{{Y}}', '{Y}'), date('Y'), $layout->post_content);
			trx_addons_sc_stack_push('show_layout');
			echo strpos($content, '[vc_row')!==false
				? do_shortcode($content)
				: apply_filters('trx_addons_filter_sc_layout_content', $content, $layout->ID);
