<?php
/*
* Define class psp_BuddyPress
* Make sure you skip down to the end of this file, as there are a few
* lines of code that are very important.
*/
!defined('ABSPATH') and exit;
if (class_exists('psp_BuddyPress') != true) {
	class psp_BuddyPress {
		...
		*/
		public function __construct( $parent ) {
			...
			$this->components = array(
				'groups' => array(
					'title' 	=>esc_html__('Group Directory', 'smartSEO'),
					'std'		=> '',
					'tags'		=> '',
					'actions'	=> array(
						'home' => array(
							'title' 	=>esc_html__('Group Home', 'smartSEO'),
							'std'		=> '',
							'tags'		=> ''
						),
						'forum' => array(
							'title' 	=>esc_html__('Group Forum', 'smartSEO'),
							'std'		=> '',
							'tags'		=> ''
						),
						'forum-topic' => array(
							'title' 	=>esc_html__('Group Forum Topic', 'smartSEO'),
							'std'		=> '',
							'tags'		=> ''
						),
						'members' => array(
							'title' 	=>esc_html__('Group Members', 'smartSEO'),
							'std'		=> '',
							'tags'		=> ''
						),
						'my-groups' => array(
							'title' 	=>esc_html__('Profile Groups', 'smartSEO'),
							'std'		=> '',
							'tags'		=> ''
						)
					)
				...
				'activity' => array(
					'title' 	=>esc_html__('Activity Directory', 'smartSEO'),
					'std'		=> '',
					'tags'		=> '',
					'actions'	=> array(
						'groups' => array(
							'title' 	=>esc_html__('Group Activity', 'smartSEO'),
							'std'		=> '',
							'tags'		=> ''
						),
						'activity' => array(
							'title' 	=>esc_html__('Activity Home', 'smartSEO'),
							'std'		=> '',
							'tags'		=> ''
						),
						'mentions' => array(
							'title' 	=>esc_html__('Activity Mentions', 'smartSEO'),
							'std'		=> '',
							'tags'		=> ''
						),
						'favorites' => array(
							'title' 	=>esc_html__('Activity Favorites', 'smartSEO'),
							'std'		=> '',
							'tags'		=> ''
						),
						'friends' => array(
							'title' 	=>esc_html__('Activity Friends', 'smartSEO'),
							'std'		=> '',
							'tags'		=> ''
						),
						'just-me' => array(
							'title' 	=>esc_html__('Profile Activity', 'smartSEO'),
							'std'		=> '',
							'tags'		=> ''
						)
					)
				...
				'members' => array(
					'title' 	=>esc_html__('Members Directory', 'smartSEO'),
					'std'		=> '',
					'tags'		=> '',
					'actions'	=> array()
				),
				'profile' => array(
					'actions'	=> array(
						'public' => array(
							'title' 	=>esc_html__('Profile Home', 'smartSEO'),
							'std'		=> '',
							'tags'		=> ''
						)
					)
				...
				'blogs' => array(
					'title' 	=>esc_html__('Blogs Directory', 'smartSEO'),
					'std'		=> '',
					'tags'		=> '',
					'actions'	=> array(
						'my-blogs' => array(
							'title' 	=>esc_html__('Profile Blogs', 'smartSEO'),
							'std'		=> '',
							'tags'		=> ''
						)
					)
				...
				'forums' => array(
					'actions'	=> array(
						'topics' => array(
							'title' 	=>esc_html__('Profile Forums', 'smartSEO'),
							'std'		=> '',
							'tags'		=> ''
						),
						'replies' => array(
							'title' 	=>esc_html__('Profile Forums Replies', 'smartSEO'),
							'std'		=> '',
							'tags'		=> ''
						),
						'favorites' => array(
							'title' 	=>esc_html__('Profile Forums Favorites', 'smartSEO'),
							'std'		=> '',
							'tags'		=> ''
						)
					)
				...
				'friends' => array(
					'actions'	=> array(
						'my-friends' => array(
							'title' 	=>esc_html__('Profile Friends', 'smartSEO'),
							'std'		=> '',
							'tags'		=> ''
						)
					)
			...
			$this->components_noadmin = array(
				'register' => array(
					'title' 	=>esc_html__('Create an Account', 'smartSEO'),
					'std'		=> ''
				),
				'activate' => array(
					'title' 	=>esc_html__('Activate your Account', 'smartSEO'),
					'std'		=> ''
				),
				'search' => array(
					'title' 	=>esc_html__('Search', 'smartSEO'),
					'std'		=> ''
				)
			);
		...
		*/
		...
		public function build_components() {
			...
			if ( is_array( $bp->active_components ) ) {
				...
				foreach ( $components as $key ) {
					$keyy = $key;
					if ( in_array($key, array_keys($conversion)) ) {
						$keyy = $conversion["$key"];
					}
		...
		public function is_component_active( $slug ) {
			...
			if ( !empty($componentsList) && is_array($componentsList) ) {
				if ( in_array( $component_name, $componentsList ) ) {
					return true;
				}
			}        
			return false;
				...
				if ( !empty($component) ) {
					if ( !empty($action) ) {
						...
						if ( $this->verifyPageSlugIsSet($component, $action) ) {
							...
							$this->add_dynamic_pagetype($page_type);
						}
					} else {
		...
		public function build_admin_options() {
			...
			$ret = array(
				...
				,'subtabs'	=> array(
					'__tab1'	=> array(
						'__subtab2' => array(
							esc_html__('Buddy Press', 'smartSEO'), 'bp_help_format_tags')),
					'__tab2'	=> array(
						'__subtab4' => array(
							esc_html__('Buddy Press', 'smartSEO'), '')),
					'__tab3'	=> array(
						'__subtab4' => array(
							esc_html__('Buddy Press', 'smartSEO'), '')),
					'__tab4'	=> array(
				...
				if ( !empty($v) ) {
					foreach ($v as $kk => $vv) {
						...
						foreach ( array_keys($metatags) as $kk2 => $vv2) {
							...
							// type
							$type = 'text';
							if ( in_array($vv2, array('desc')) ) {
								$type = 'textarea';
							} else if ( in_array($vv2, array('robots')) ) {
								$type = 'multiselect';
							}
							...
							// title
							$title = $theVal['title'] . ' ' . $metatags["$vv2"];
							
							// description
							$desc = array();
							$desc[] = sprintf(esc_html__('Component: <u>%1$s</u> ; Action: <u>%2$s</u>', 'smartSEO'), $component, ( !empty($action) ? $action : $action ));
		...
		public function buildFromPageSlug( $page_type ) {
			...
			if ( isset($page_type2[0]) ) {
				$ret['component'] = $page_type2[0];
			}
			if ( isset($page_type2[1]) ) {
				$ret['action'] = $page_type2[1];
			}
...
// Initialize the psp_BuddyPress class
//$psp_BuddyPress = new psp_BuddyPress();
