 ...
 * ===============================+
 *
 * @package		psp_AjaxListTable
 * @author		Andrei Dinca
 * @version		1.0
 */
! defined( 'ABSPATH' ) and exit;

if (class_exists('psp_AjaxListTable') != true) {
	class psp_AjaxListTable {
		...
		public function request() {
			$request = array(
				...
				'ajax_id' 		=> isset($_REQUEST['ajax_id']) ? sanitize_text_field($_REQUEST['ajax_id']) : '',
				'params' 		=> isset($_REQUEST['params']) ? sanitize_text_field($_REQUEST['params']) : '',
			);
		...
		private function get_list_postTypes() {
			...
			if ( !isset($this->opt['list_post_types']) ) {
				$this->opt['list_post_types'] = 'all';
			}
		...
		public function post_statuses_filter() {
			...
			if ( $this->opt['post_statuses'] == 'all' ) {
				$postStatuses = array(
					'all'   	=>esc_html__('All', 'smartSEO'),
					'publish'   =>esc_html__('Published', 'smartSEO'),
					'draft'   	=>esc_html__('Draft', 'smartSEO'),
					'future'    =>esc_html__('Scheduled', 'smartSEO'),
					'private'   =>esc_html__('Private', 'smartSEO'),
					'pending'   =>esc_html__('Pending Review', 'smartSEO')
				);
			} else {
		...
		private function post_media_status() {
			...
			$postStatuses = array(
				'all'   			=>esc_html__('All', 'smartSEO'),
				'smushed'   		=>esc_html__('Smushed', 'smartSEO'),
				'not_processed'   	=>esc_html__('Not processed', 'smartSEO'),
				'with_errors'   	=>esc_html__('With errors', 'smartSEO')
			);
			return $postStatuses;
		}
		...
		public function post_media_filter( $return = 'output' ) {
			...
			foreach ($postStatuses as $key => $value) {
				...
				if ( $return == 'array' && $key == 'all' ) {
					unset($postStatuses[$key]);
				}
		...
		private function get_pagination() {
			...
			if ( $this->opt['show_pagination'] ) {
				...
				$html[] =				esc_html__('Show All', 'smartSEO');
				$html[] = 				'</option>';
				$html[] =			'</select>';
				$html[] = 			'<label for="psp-post-per-page">' .esc_html__('per page', 'smartSEO') . '</label>';
				$html[] = 		'</div>';

				$html[] = 		'<div class="psp-list-table-pagination tablenav">';

				$html[] = 			'<div class="tablenav-pages">';
				$html[] = 				'<span class="displaying-num">' . ( $this->items_nr ) . ' ' .esc_html__('items', 'smartSEO') . '</span>';
				if ( $total_pages > 1 ) {

					$html[] = 				'<span class="pagination-links"><a class="first-page ' . ( $paged == 1 ? 'disabled' : '' ) . ' psp-jump-page" title="' .esc_html__('Go to the first page', 'smartSEO') . '" href="#paged=1">«</a>';
					$html[] = 				'<a class="prev-page ' . ( $paged == 1 ? 'disabled' : '' ) . ' psp-jump-page" title="' .esc_html__('Go to the previous page', 'smartSEO') . '" href="#paged=' . ( $paged > 2 ? ( $paged - 1 ) : 1 ) . '">‹</a>';
					$html[] = 				'<span class="paging-input"><input class="current-page" title="' .esc_html__('Current page', 'smartSEO') . '" type="text" name="paged" value="' . ( $paged ) . '" size="2" style="width: 45px;"> ' .esc_html__('of', 'smartSEO') . ' <span class="total-pages">' . ( ceil( $this->items_nr / $this->args['posts_per_page'] ) ) . '</span></span>';
					$html[] = 				'<a class="next-page ' . ( ( $paged == ( $total_pages ) ) ? 'disabled' : '' ) . ' psp-jump-page" title="' .esc_html__('Go to the next page', 'smartSEO') . '" href="#paged=' . ( $paged < $total_pages ? $paged + 1 : $total_pages ) . '">›</a>';
					$html[] = 				'<a class="last-page ' . ( $paged ==  ( $total_pages - 1 ) ? 'disabled' : '' ) . ' psp-jump-page" title="' .esc_html__('Go to the last page', 'smartSEO'	) . '" href="#paged=' . ( $total_pages ) . '">»</a></span>';
				}
				$html[] = 			'</div>';
				$html[] = 		'</div>';
		...
		public function print_header() {
			...
			if ( trim($this->opt['custom_table']) == '') {
				...
				if ( $this->opt['search_box'] ) {
					...
					$html[] = 			'<input type="button" name="psp-search-btn" id="psp-search-btn" class="button" value="' .esc_html__('Search Posts', 'smartSEO') . '">';
					$html[] = 		'</div>';
					$html[] = 	'</div>';
					$nb_cols++;
				}
				...
				if (1) {
					...
					foreach ($filter_fields as $field => $vals) {
						...
						if ( $include_all ) { // && count($options) > 1
							...
							$options = array_replace(array(), array(
								'--all--' 		=>esc_html__('Show All', 'smartSEO'),
							), $options);
						}
					...
					if ( !empty($search_box) ) {
						...
						$search_title = isset($search_box['title'])
							? $search_box['title'] :esc_html__('Search', 'smartSEO');
						...
						$html[] = 		'<div class="psp-list-table-search-box psp-search-standard-design">';
						$html[] = 			'<input type="text" name="psp-search-text" id="psp-search-text" value="' . ( $search_text ) . '" class="' . ( $search_text!='' ? 'search-highlight' : '' ) . '" />';
						$html[] = 			'<input type="button" name="psp-search-btn" id="psp-search-btn" class="psp-form-button-small psp-form-button-primary" value="' . $search_title . '" />';
						$html[] = 		'</div>';
						$html[] = 	'</div>';
						$nb_cols++;
					} // end search box
			...
			if ( $this->opt['show_header_buttons'] ) {
				if ( isset($this->opt['mass_actions']) && ( $this->opt['mass_actions'] === false ) ) {
					...
					$html[] = 	'<input type="button" value="' .esc_html__('Auto detect focus keyword for All', 'smartSEO') . '" id="psp-all-auto-detect-kw" class="psp-form-button-small psp-form-button-info">';
					$html[] = 	'<input type="button" value="' .esc_html__('Optimize All', 'smartSEO') . '" id="psp-all-optimize" class="psp-form-button-small psp-form-button-info">';
					$html[] = '</div>';
				}
			...
			foreach ($this->items as $post) { // main foreach
				...
				foreach ($this->opt['columns'] as $key => $value) { // columns foreach
					...
					if ( $value['td'] == 'checkbox' ) {
						...
						//$html[] = '<input type="button" class="psp-auto-detect-kw-btn psp-form-button-small psp-form-button-info" value="' .esc_html__('Auto detect', 'smartSEO') . '" />';
						$html[] = '</div>';
						...
						$html[] = '<a class="psp-button green psp-seo-report-btn psp-form-button-small psp-form-button-success" href="#" data-itemid="' . ( $post->ID ) . '">
                                    	' .esc_html__('SEO Report', 'smartSEO') . '
                                    </a>';
						...
						if ( $post->post_status == 'inherit' && $post->post_type == 'attachment' ) { // media image file
							...
							<span class="psp-inline-row-actions show" id="psp-inline-row-actions-' . ( $post->ID ) . '">
								<a href="' . ( sprintf( admin_url('post.php?post=%s&action=edit'), $post->ID) ) . '">Edit</a>
								 | <a href="' . ( wp_get_attachment_url( $post->ID ) ) . '" target="_blank">' .esc_html__('View', 'smartSEO') . '</a>
							</span>';
						} else {
						...
						<span class="psp-inline-row-actions show" id="psp-inline-row-actions-' . ( $post->ID ) . '">
							<a href="' . ( sprintf( admin_url('post.php?post=%s&action=edit'), $post->ID) ) . '">Edit</a>
							 | <a href="#" class="editinline" title="' .esc_html__('Edit this item inline', 'smartSEO') . '">' .esc_html__('Quick Edit', 'smartSEO') . '</a>
							 | <a href="' . ( get_permalink( $post->ID ) ) . '" target="_blank">' .esc_html__('View', 'smartSEO') . '</a>
						</span>';
						$html[] = '
						...
						$html[] = ( trim($post['referrers']) != '' ? '<a href="#referrers" class="psp-button gray psp-btn-referrers-lightbox" data-itemid="' . ( $post['id'] ) . '">' . (esc_html__('Show All', 'smartSEO') ) . '</a>' : '-' );
					} elseif ( $value['td'] == '%user_agents%' ) {
						$html[] = ( trim($post['user_agents']) != '' ? '<a href="#user_agents" class="psp-button gray psp-btn-user_agents-lightbox" data-itemid="' . ( $post['id'] ) . '">' . (esc_html__('Show All', 'smartSEO') ) . '</a>' : '-' );
						...
						if ( $value['td'] == '%thumbnail%' ) {
							...
							if ( isset($meta_new['psp_smushit']) && !empty($meta_new['psp_smushit']) ) {
								...
								if ( isset($meta_new['psp_smushit_errors']) && ( (int) $meta_new['psp_smushit_errors'] ) > 0 ) {
									...
									$__msg = array(esc_html__('errors occured during smushit operation!', 'smartSEO') );
								} else {
									$status = 'valid';
								...
								$html[] = '<div id="' . ( 'psp-smushit-resp-' . $id ) . '" class="psp-message psp-info">' . esc_html__('not processed!', 'smartSEO' ) . '</div><br />';
							}
							//$html[] = '</div>';
					...
					if ( $this->opt['id'] == 'pspLinkBuilder' ) {
						...
						if ( $value['td'] == '%builder_phrase%' ) {
							...
							$html[] = ( 1==1 ? '<a href="#url_attributes" class="psp-button gray psp-btn-url-attributes-lightbox" data-itemid="' . ( $post['id'] ) . '">' . (esc_html__('Show All', 'smartSEO') ) . '</a>' : '-' );
					...
					if ( $this->opt['id'] == 'pspLinkRedirect' ) {
						if ( $value['td'] == '%linkred_url%' ) {
							...
							$redirect_rules = array(
								'custom_url' =>esc_html__('Custom URL', 'smartSEO'),
								'regexp' =>esc_html__('Regexp', 'smartSEO'),
							);
							...
							$last_status =esc_html__('Never Checked', 'smartSEO');
							$last_css_class = 'psp-message psp-info';
							if ( isset($post['redirect_rule']) && ( 'regexp' == $post['redirect_rule'] ) ) {
								$last_status =esc_html__('**', 'smartSEO');
								$last_css_class = '';
							}

							if ( 'valid' == $target_code ) {
								$last_status =esc_html__('Valid', 'smartSEO');
								$last_css_class = 'psp-message psp-success';
							} else if ( 'invalid' == $target_code ) {
								$last_status =esc_html__('Invalid', 'smartSEO');
					...
					if ( $this->opt['id'] == 'pspWebDirectories' ) {
						if ( $value['td'] == '%directory_name%' ) {
							...
							$html[] = '<a href="' . ( $post['submit_url'] ) . '" target="_blank" class="psp-form-button psp-form-button-info psp-btn-submit-website" data-itemid="' . ( $post['id'] ) . '">' . (esc_html__('Submit website', 'smartSEO') ) . '</a>';
						} elseif ( $value['td'] == '%submit_status%' ) {
							// never submited / $post['status'] = 2;
							$html_status = '<div class="psp-message" style="padding: 5px;">' . (esc_html__('Never submited', 'smartSEO') ) . '</div>';
							if ( $post['status'] == 2 ) {
								$html_status = '<div class="psp-message psp-warning" style="padding: 5px;background-image: none;">' . (esc_html__('Submit in progress', 'smartSEO') ) . '</div>';
							} elseif ( $post['status'] == 3 ) {
								$html_status = '<div class="psp-message psp-error" style="padding: 5px;background-image: none;">' . (esc_html__('Error on submit', 'smartSEO') ) . '</div>';
							} elseif ( $post['status'] == 1 ) {
								$html_status = '<div class="psp-message psp-success" style="padding: 5px;background-image: none;">' . (esc_html__('Submit successfully', 'smartSEO') ) . '</div>';
							}
					...
					if ( $this->opt['id'] == 'pspPageHTMLValidation' ) {
						...
						if ( $value['td'] == '%nr_of_errors%' ) {
							...
							$html[] = '<a target="_blank" href="' . ( 'http://validator.w3.org/check?uri=' . get_permalink( $post->ID ) ) . '" class="psp-button gray">' . (esc_html__('View report', 'smartSEO') ) . '</a>';
						}
					}
					...
					if ( $this->opt['id'] == 'pspSERPKeywords' ) {
						...
						if ( $value['td'] == '%serp_focus_keyword%' ) {
							...
							if ( isset($rank_data) && is_array($rank_data) && count($rank_data) > 0 ) {
								...
								$__notInTop100 =esc_html__('Not in top', 'smartSEO');
						...
						if ( $value['td'] == '%post_id%' ) {
							...
							<span class="psp-inline-row-actions show" id="psp-inline-row-actions-' . ( $post['id'] ) . '">
								<a href="' . ( sprintf( admin_url('post.php?post=%s&action=edit'), $__postInfo->ID) ) . '">Edit</a>
								 | <a href="' . ( get_permalink( $__postInfo->ID ) ) . '" target="_blank">' .esc_html__('View', 'smartSEO') . '</a>
							</span>';
						} else if ( $value['td'] == '%status%' ) {

							$__statusVals = array(
								0 	=> esc_html__('New', 'smartSEO' ),
								1	=> esc_html__('Finished', 'smartSEO' ),
								2	=> esc_html__('Running', 'smartSEO' ),
								3	=> esc_html__('Error', 'smartSEO' )
							);
							...
							$__statusVals = array(
								'on' 	=> esc_html__('ON', 'smartSEO' ), 
								'off'	=> esc_html__('OFF', 'smartSEO' )
							);
							...
							$__statusVals = array(
								'EVERYONE' =>esc_html__('Everyone', 'smartSEO'),
								'ALL_FRIENDS' =>esc_html__('All Friends', 'smartSEO'),
								'NETWORKS_FRIENDS' =>esc_html__('Networks Friends', 'smartSEO'),
								'FRIENDS_OF_FRIENDS' =>esc_html__('Friends of Friends', 'smartSEO'),
								'CUSTOM' =>esc_html__('Private (only me)', 'smartSEO')
							);
		...
		public function __action_publish() {
			...
			$request = array(
				'itemid' 	=> isset($_REQUEST['itemid']) ? (int) sanitize_text_field($_REQUEST['itemid']) : 0,
			);
		...
		public function __action_bulk_delete() {
			...
			$request = array(
				'id' 			=> isset($_REQUEST['id']) && !empty(sanitize_text_field($_REQUEST['id']))
					? trim(sanitize_text_field($_REQUEST['id'])) : 0
			);
		...
		public function __action_edit_inline() {
			...
			$request = array(
				'table'			=> isset($_REQUEST['table']) ? trim((string) sanitize_text_field($_REQUEST['table'])) : '',
				'itemid' 		=> isset($_REQUEST['itemid']) ? (int) sanitize_text_field($_REQUEST['itemid']) : 0,
				'field_name'	=> isset($_REQUEST['field_name']) ? trim((string) sanitize_text_field($_REQUEST['field_name'])) : '',
				'field_value'	=> isset($_REQUEST['field_value']) ? trim((string) sanitize_text_field($_REQUEST['field_value'])) : '',
			);
			extract($request);
