...
if (class_exists('psp') != true) {
	class psp {
		...
		private static $plugin_row_meta = array(
			'buy_url'			=> 'https://codecanyon.net/item/smart-seo-wordpress-plugin/224446',
			'docs_url'			=> 'http://docs.aa-team.com/smartseo/',
		...
		public function __construct( $here = __FILE__) {
			...
			$this->buildConfigParams('freamwork-css-files', array(
				...
				'fixed-inline' => 'css/fixed-inline.css'
			));
			...
			if ( $this->is_admin ) {
				...
				if ( get_option('psp_depedencies_is_valid', false) ) {
					...
					$this->pluginDepedencies = new psp_aaTeamPluginDepedencies( $this );
			...
			if ( !$this->is_admin ) {
				$ispspreq = isset($_POST['ispspreq']) ? sanitize_text_field($_POST['ispspreq']) : null;
				if ( ! is_null($ispspreq) && in_array( $ispspreq, array('tax', 'post') ) ) {
					if ( 'post' === $ispspreq ) {
						add_filter( 'the_content', array( $this, 'mark_content' ), 0, 1 );
					} else if ( 'tax' === $ispspreq ) {
		...
		public function print_section_header( $title = '', $desc = '', $docs_url = '') {
			...
			if ( trim($title) != '' ) {
				$html[] =       '<h1 class="panel-title ' . ( $this->alias ) . '-panel-title">' . ( $title ) . '</h1>';
			}
			if ( trim($desc) != '' ) {
				$html[] =       $desc;
			}
			...
			if ( trim($docs_url) != '' ) {
				$html[] =       '<a href="' . ( $docs_url ) . '" target="_blank" class="' . ( $this->alias ) . '-tab"><span class="psp-icon-support"><span class="path1"></span><span class="path2"></span><span class="path3"></span><span class="path4"></span><span class="path5"></span></span> Documentation</a>';
			}
		...
		public function session_start() {
			...
			#$session_id = isset($_COOKIE['PHPSESSID']) ? @session_id($_COOKIE['PHPSESSID']) : ( isset($_REQUEST['PHPSESSID']) ? $_REQUEST['PHPSESSID'] : @session_id() );
				...
				// skip session init if on site health page
				if( !strpos( $_SERVER['REQUEST_URI'], 'site-health.php') && (isset($_SERVER['HTTP_REFERER']) && !strpos( $_SERVER['HTTP_REFERER'], 'site-health.php') ) ){
					@session_start();
				}
			}
		...
		*/
		...
		public function getPageContent( $post = null, $oldcontent = '', $istax = false ) { 
			...
			if ( is_null($post) || ( !is_object($post) && !is_array($post) ) ) {
				return $oldcontent;
			}
			if ( $istax ) {
				if ( is_object($post) && !isset($post->term_id) ) {
					return $oldcontent;
				}
				if ( is_array($post) && !isset($post['term_id']) ) {
					return $oldcontent;
				}
			} else {
		...
		public function admin_notice_install_styles() {
			if ( !wp_style_is('psp-activation') ) {
				wp_enqueue_style( 'psp-activation', $this->cfg['paths']['freamwork_dir_url'] . 'css/activation.css');
			}
		...
		public function plugin_redirect() {
			...
			$req = array(
				...
				'page'						=> isset($_REQUEST['page']) ? (string) sanitize_text_field($_REQUEST['page']) : '',
			);
			extract($req);
		...
		public function save_options () {
			...
			die(json_encode( array(
				'status' => 'ok',
				'html' 	 =>esc_html__('Options updated successfully', 'smartSEO')
			)));
		}
		...
		public function install_default_options () {
			...
			if (count($pullOutArray) == 0) {
				die(json_encode( array(
					'status' => 'error',
					'html' 	 =>esc_html__('Invalid install default json string, can\'t parse it!', 'smartSEO')
				)));
			} else {
				...
				die(json_encode( array(
					'status' => 'ok',
					'html' 	 =>esc_html__('Install default successful', 'smartSEO')
				)));
			}
		}
		...
		public function module_change_status ( $resp = 'ajax' ) {
			...
			$db_alias = $this->alias . '_module_' . sanitize_text_field($_REQUEST['module']);
			update_option( $db_alias, sanitize_text_field($_REQUEST['the_status']) );
			
			if ( sanitize_text_field($_REQUEST['module']) == 'facebook_planner' ) {
				if ( sanitize_text_field($_REQUEST['the_status']) == 'true' ) {
				...
				} else if ( sanitize_text_field($_REQUEST['the_status']) == 'false' ) {
		...
		public function module_bulk_change_status () {
			...
			$request = array(
				'id' 			=> isset($_REQUEST['id']) && !empty(sanitize_text_field($_REQUEST['id'])) ? trim(sanitize_text_field($_REQUEST['id'])) : ''
			);
		...
		public function load_section () {
			$request = array(
				'section' 		=> isset($_REQUEST['section']) ? strip_tags(sanitize_text_field($_REQUEST['section'])) : false,
				'subsection' 	=> isset($_REQUEST['subsection']) ? strip_tags(sanitize_text_field($_REQUEST['subsection'])) : false
			);

			// get module if isset
			if (!in_array( $request['section'], $this->cfg['activate_modules'])) {
				die(json_encode(array('status' => 'err', 'msg' =>esc_html__('invalid section want to load!', 'smartSEO'))));
			}
		...
		*/
			...
			if ( isset($this->cfg['freamwork-css-files'])
				...
				foreach ($this->cfg['freamwork-css-files'] as $key => $value) {
					if ( is_file($this->cfg['paths']['freamwork_dir_path'] . $value) ) {
						...
						$this->errors->add( 'warning', sprintf(
							esc_html__('Invalid CSS path to file: <strong>%s</strong>. Call in: %s' , 'smartSEO'),
							$this->cfg['paths']['freamwork_dir_path'] . $value,
							__FILE__ . ':' . __LINE__
						));
					}
				}
		...
		public function admin_load_styles() {
			...
			wp_enqueue_style( 'psp-google-Roboto', $protocol . '://fonts.googleapis.com/css?family=Roboto:400,500,400italic,500italic,700,700italic' );
			//wp_enqueue_style( 'psp-font-awesome', $protocol . '://maxcdn.bootstrapcdn.com/font-awesome/4.6.2/css/font-awesome.min.css' );
			wp_enqueue_style( 'psp-font-awesome', $this->cfg['paths']['freamwork_dir_url'] . 'font-awesome/5.15.3/font-awesome.min.css', array(), '5.15.3' );
			wp_enqueue_style( 'psp-admin-font', $this->cfg['paths']['freamwork_dir_url'] . 'css/font.css' );
			wp_enqueue_style( 'psp-seo-checks', $this->cfg['paths']['freamwork_dir_url'] . 'css/seo-checks.css' );
			...
			wp_enqueue_style( 'psp-main-style', $main_style, array( 'psp-font-awesome' ) );
		...
		*/
		...
		public function admin_load_scripts() {
			...
			if ( count($javascript) > 0 ) {
				...
				if ( in_array( 'thickbox', $javascript ) ) {
					wp_enqueue_script( 'psp-thickbox' );
				}
	
				// date & time picker
				if ( !wp_script_is('psp-jquery-timepicker') ) {
					if ( in_array( 'jquery-timepicker', $javascript ) ) {
						wp_enqueue_script( 'psp-jquery-timepicker' , $this->cfg['paths']['freamwork_dir_url'] . 'js/jquery.timepicker.v1.1.1.min.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-datepicker', 'jquery-ui-slider' ) );
					}
				}
				
				// star rating - rateit
				if ( !wp_script_is('psp-jquery-rateit-js') ) {
					if ( in_array( 'jquery-rateit-js', $javascript ) ) {
						
						if ( !wp_style_is('psp-jquery-rateit-css') ) {
							wp_enqueue_style( 'psp-jquery-rateit-css' , $this->cfg['paths']['freamwork_dir_url'] . 'js/rateit/rateit.css' );
						}
						wp_enqueue_script( 'psp-jquery-rateit-js' , $this->cfg['paths']['freamwork_dir_url'] . 'js/rateit/jquery.rateit.min.js', array( 'jquery' ) );
					}
				}
			}
				...
				foreach ($this->cfg['freamwork-js-files'] as $key => $value) {
					...
					if ( is_file($this->cfg['paths']['freamwork_dir_path'] . $value) ) {
						if ( in_array( $key, $javascript ) ) {
							$_script_key = $this->alias . '-' . $key;
							wp_enqueue_script( $_script_key, $this->cfg['paths']['freamwork_dir_url'] . $value . '?' . time() );
						}
					} else {
						$this->errors->add( 'warning', sprintf(
							esc_html__('Invalid JS path to file: <strong>%s</strong> . Call in: %s' , 'smartSEO'),
							$this->cfg['paths']['freamwork_dir_path'] . $value,
							__FILE__ . ':' . __LINE__
						));
					}
		...
		public function createDashboardPage () {
			if ( $this->capabilities_user_has_module('dashboard') ) {
				...
				add_menu_page(
					esc_html__('smartSEO - Dashboard', 'smartSEO' ),
					esc_html__('smartSEO', 'smartSEO' ),
					'read',
					$this->alias,
		...
		*/
		public function load_modules( $pluginPage = '' ) {
			...
			foreach ($modules_list as $module_config ) {
				...
				//var_dump('<pre>',$module_config,$settings,'</pre>');
		...
		public function print_psp_usages() {
			...
			// !!! css moved in aa-framework/css/fixed-inline.css
		...
		*/
		public function upload_file () {
			$slider_options = '';
			 // Acts as the name
			$clickedID = isset($_POST['clickedID']) ? sanitize_text_field($_POST['clickedID']) : null;

			// Upload
			if ( 'upload' === sanitize_text_field($_POST['type']) ) {
				$override['action'] = 'wp_handle_upload';
				$override['test_form'] = false;
				$filename = sanitize_file_name( $_FILES [$clickedID] );
				...
				if (!empty($uploaded_file['error'])) {
					...
					die( json_encode(array(
							...
							'thumb' => $this->image_resize( $uploaded_file['url'], sanitize_text_field($_POST['thumb_w']), sanitize_text_field($_POST['thumb_h']), sanitize_text_field($_POST['thumb_zc']) )
						)
					) );
				} // Is the Response
			} else {
		...
		public function wp_media_upload_image() {
			$image = wp_get_attachment_image_src( (int) sanitize_text_field($_REQUEST['att_id']), 'thumbnail' );
			die(json_encode(array(
				'status' 	=> 'valid',
				'thumb'		=> $image[0]
			)));
		...
		*/
		...
		public function edit_post_inline_boxtpl() {
			...
			foreach ($__range2 as $kk => $vv) {
				...
				$vv = (string) $vv;
				if ( $vv=='1' ) {
					$__priorityText = ' - ' .esc_html__('Highest priority', 'smartSEO');
				} else if ( $vv=='0.5' ) {
					$__priorityText = ' - ' .esc_html__('Medium priority', 'smartSEO');
				} else if ( $vv=='0.1' ) {
					$__priorityText = ' - ' .esc_html__('Lowest priority', 'smartSEO');
				}
	...
	<table class="psp-inline-edit-post form-table" style="border: 1px solid #dadada;">
		<thead>
			<tr>
				<th>' .esc_html__('Meta Description', 'smartSEO') . '</th>
				<th>' .esc_html__('Meta Keywords', 'smartSEO') . '</th>
			</tr>
		</thead>
		<tbody>
			<tr>
							...
							<tr>
								<td>
									<div class="psp-inline-meta-wrapp">
										<div>' .esc_html__('Meta Robots Index:', 'smartSEO') . '</div>
										<select name="psp-editpost-meta-robots-index" id="psp-editpost-meta-robots-index">
											<option value="default" selected="true">' .esc_html__('Default Setting', 'smartSEO') . '</option>
											<option value="index">' .esc_html__('Index', 'smartSEO') . '</option>
											<option value="noindex">' .esc_html__('NO Index', 'smartSEO') . '</option>
										</select>
									</div>
								</td>
								<td>
									<div class="psp-inline-meta-wrapp">
										<div>' .esc_html__('Meta Robots Follow:', 'smartSEO') . '</div>
								...
								<td>
									<div class="psp-inline-meta-wrapp">
										<div>' .esc_html__('Include in Sitemap:', 'smartSEO') . '</div>
										<select name="psp-editpost-include-sitemap" id="psp-editpost-include-sitemap">
											<option value="default" selected="true">' .esc_html__('Default Setting', 'smartSEO') . '</option>
											<option value="always_include">' .esc_html__('Always include', 'smartSEO') . '</option>
											<option value="never_include">' .esc_html__('Never include', 'smartSEO') . '</option>
										</select>
									</div>
								</td>
								<td>
									<div class="psp-inline-meta-wrapp">
										<div>' .esc_html__('Sitemap Priority:', 'smartSEO') . '</div>
			...
			<tr>
				<td colspan=3>
					<div style="float:left; width:100%;">
						<input type="button" value="' .esc_html__('Cancel', 'smartSEO') . '" id="psp-inline-btn-cancel" class="psp-button gray" style="float:left;">
						<input type="button" value="' .esc_html__('Save', 'smartSEO') . '" id="psp-inline-btn-save" class="psp-button blue psp-form-button-small psp-form-button-info" style="float:right;">
					</div>
				</td>
			</tr>
		</tbody>
		...
		public function smushit_show_sizes_msg_details( $meta = array(), $show_sizes = true ) {
			...
			foreach ( $meta['sizes'] as $key => $val ) {
				...
				if ( !in_array($key, $selected_sizes) ) {
					continue 1;
				}
		...
		public function loadRichSnippets( $section = 'init' ) {
			...
			if ( !in_array($section, array('init', 'options')) ) {
				return false;
			}
		...
		<div id="message" class="updated aaFrm-message_activate wc-connect">
			<div class="squeezer">
				<h4><?php esc_html_e('AA Social Share notice: you already use smartSEO - Social Stats module, Social Sharing section', 'smartSEO' ); ?></h4>
			</div>
		</div>
			<?php	
		}
		...
		public function is_buddypress() {
			...
			if ( $bp->maintenance_mode == 'install' ) {
				if ( 'psp' === sanitize_text_field($_GET['page']) ) {
					$this->errors = esc_html__('The Buddypress installation it\'s not finished!', 'smartSEO' );
		...
		public function bp_warning_box() {
			?>
		<div class="updated"> <p><?php esc_html_e('<strong>smartSEO</strong> | ', 'smartSEO' ); ?><?php echo $this->errors; ?></p> </div>
			<?php
		}
		...
		public function get_php_ini_bool( $value) {
			...
			return in_array($value, array('+', '1', 'y', 'on', 'yes', 'true', 'enabled')) ? true : ( in_array($value, array('-', '0', 'n', 'off', 'no', 'false', 'disabled')) ? false : (bool) $value );
		}
		...
		public function get_client_utils() {
			...
			$mobileDetect = new psp_aaMobile_Detect();

			$utils['isMobile'] = $mobileDetect->isMobile();
			$utils['device_type'] = $mobileDetect->type();
			if ( $utils['device_type'] == 'tablet') {
				$utils['device_type'] = 'mobile';
			}
		...
		private function check_if_table_exists( $force = false ) {
			...
			if ( empty($dbTables) || ! is_array($dbTables) ) {
				...
				$this->plugin_integrity_update_time('check_tables', array(
					'status'		=> 'invalid',
					'html'		=>esc_html__('Check plugin tables: error requesting tables list.', 'smartSEO'),
				));
			...
			if ( ! $dbTables_missing ) {
				...
				$this->plugin_integrity_update_time('check_tables', array(
					'timeout'	=> time(),
					'status'		=> 'valid',
					'html'		=> sprintf(esc_html__('Check plugin tables: all installed ( %s ).', 'smartSEO'), implode(', ', $dbTables_found) ),
				));
			...
			$this->plugin_integrity_update_time('check_tables', array(
				'status'		=> 'invalid',
				'html'		=> sprintf(esc_html__('Check plugin tables: missing ( %s ).', 'smartSEO'), implode(', ', $dbTables_missing) ),
			));
			return false; //something was wrong!
		}
		...
		public function _update_db_tables( $pms = array() ) {
			...
			if ( isset($indexes) ) {
				foreach ( (array) $indexes as $skey => $sql ) {
					if ( ! isset($sql['main']) ) {
						continue 1;
					}
			...
			$this->plugin_integrity_update_time('check_alter_tables', array(
				'timeout'	=> time(),
				'status'		=> 'valid',
				'html'		=>esc_html__('Check plugin tables (alter): OK.', 'smartSEO'),
			));
		}
		...
		public function facebook_call_plugin_default() {
			...
			$def['plugin_url_'] = '<a href="' . $def['plugin_url'] . '" class="psp-form-button-small psp-form-button-info">' .esc_html__('Go Back to the plugin facebook planner module and try again.', 'smartSEO') . '</a><br />';
			return $def;
		}
		...
		public function facebook_get_authorization_url( $pms = array() ) {
			...
			if ( isset($facebook) && ! is_null($facebook) ) {
				...
				$fb_permissions = array('email','publish_actions','manage_pages','publish_pages', 'user_managed_groups'); // optional
		...
		public function facebook_do_authorization( $pms = array() ) {
			...
			if ( isset($facebook) && ! is_null($facebook) ) {
				...
				if ( !empty($dbToken) ) {
					...
					if ( !empty($fb_response) ) {
						...
						if (count($user_profile) > 0) {
							...
							$html[] = '<a href="' . ( $fb_loginUrl ) . '" style="width: 133px;" class="psp-form-button-small psp-form-button-info">' . ( esc_html__('Authorize this app again', 'smartSEO' ) ) . '</a>';
				...
				if ( $validAuth == false ) {
					$html[] = '<a href="' . ( $fb_loginUrl ) . '" style="width: 84px;" type="button" class="psp-form-button-small psp-form-button-info">' . ( esc_html__('Authorizate app', 'smartSEO' ) ) . '</a>';
				}
			}
			...
			if ( !isset($accessToken) ) {
				...
				if ($is_error) {
					$error_details = array(
						...
						'error_desc'		=> $fb_helper->getErrorDescription() || ( isset($_GET['error_message']) ? sanitize_text_field($_GET['error_message'])  : null ),
					);
		...
		public function fk_missing_message( $str, $type = 'short', $markup = true ) {
			...
			if ( '' == $str ) {
				if ( 'short' == $type ) {
					$str =esc_html__('missing focus keyword', 'smartSEO');
				} else {
					$str =esc_html__('missing focus keyword - you must create one...', 'smartSEO');
				}
		...
		public function social_get_providers( $pms = array() ) {
			...
			$prv = array(
				...
				'facebook'		=> array(
					'title'			=>esc_html__('Facebook', 'smartSEO'),
					'color'			=> '#3c5b9b',
					'items_label'	=>esc_html__('shares', 'smartSEO'),
					'counts'		=> array(
						'share_count'		=> array(
							'items_label'		=>esc_html__('shares', 'smartSEO'),
						...
						'like_count'		=> array(
							'items_label'		=>esc_html__('likes', 'smartSEO'),
							'icon'				=> 'facebook-like-icon.png',
						),
						'comment_count'		=> array(
							'items_label'		=>esc_html__('comments', 'smartSEO'),
							'icon'				=> 'facebook-comments-icon.png',
						),
						//'click_count'		=> array(
						//	'items_label'		=>esc_html__('clicks', 'smartSEO'),
						//	'icon'				=> 'facebook-icon.png',
						//),
					),
				),
				'twitter'		=> array(
					'title'			=>esc_html__('Twitter', 'smartSEO'),
					'color'			=> '#00aced',
					'items_label'	=>esc_html__('retweets', 'smartSEO'),
				),
				'google'		=> array(
					'title'			=>esc_html__('Google +1', 'smartSEO'),
					'color'			=> '#d23e2b',
					'items_label'	=>esc_html__('shares', 'smartSEO'),
				),
				'pinterest'		=> array(
					'title'			=>esc_html__('Pinterest', 'smartSEO'),
					'color'			=> '#ca4638',
					'items_label'	=>esc_html__('pins', 'smartSEO'),
				),
				'stumbleupon'		=> array(
					'title'			=>esc_html__('Stumbleupon', 'smartSEO'),
					'color'			=> '#3fbd46',
					'items_label'	=>esc_html__('views', 'smartSEO'),
				),
				'linkedin'		=> array(
					'title'			=>esc_html__('Linkedin', 'smartSEO'),
					'color'			=> '#007ab9',
					'items_label'	=>esc_html__('backlinks', 'smartSEO'),
				),
				'delicious'		=> array(
					'title'			=>esc_html__('Delicious', 'smartSEO'),
					'color'			=> '#2c2c2c',
					'items_label'	=>esc_html__('posts', 'smartSEO'),
				),
				'buffer'		=> array(
					'title'			=>esc_html__('Buffer', 'smartSEO'),
					'color'			=> '#2c2c2c',
					'items_label'	=>esc_html__('posts', 'smartSEO'),
				),
				'reddit'		=> array(
					'title'			=>esc_html__('Reddit', 'smartSEO'),
					'color'			=> '#2c2c2c',
					'items_label'	=>esc_html__('posts', 'smartSEO'),
				),
				'flattr'		=> array(
					'title'			=>esc_html__('Flattr', 'smartSEO'),
					'color'			=> '#2c2c2c',
					'items_label'	=>esc_html__('posts', 'smartSEO'),
				),
			);
