<?php
/*
* Define class psp_TitleMetaFormat
* Make sure you skip down to the end of this file, as there are a few
* lines of code that are very important.
*/
!defined('ABSPATH') and exit;
if (class_exists('psp_TitleMetaFormat') != true) {
	class psp_TitleMetaFormat {
		...
		*/
		...
		protected static $tplChar = '{%s}';
		protected $pageTypes = array();
		
		private static $titleForce = false;
		...
		public function __construct() {
			...
			// 	$this->buddypress = new psp_BuddyPressTags( $this->the_plugin );
			// }
		...
		public function head_after() {
			if ($this->do_title_rewrite()) {
				...
				if ( count($handlers) > 0 && strcasecmp($handlers[ count($handlers) - 1 ], 'psp_TitleMetaFormat::head_title_tag') == 0 ) {
					ob_end_flush();
				} else {  
					// "ob_list_handlers list found:\n" . print_r($handlers, true)
				}
			}
		}
		...
		protected function the_format( $field, $type) {
			...
			if ( !is_null($__currentValue) && !empty($__currentValue) && $__currentValue!='' ) {
				...
				if ( $field == 'title' && $type != 'home' && !empty($meta_title_sufix) ) {
					$__currentValue .= ' ' . $meta_title_sufix;
				}
		...
		public function make_format( $__replace_orig = array(), $type = 'home', $theContent = '') {
			...
			$__defaults = array( //default params!
				...
				'current_month' 		=> date( 'F' ), //current month
				'current_week_day'		=> date( 'l' ), //current week day
			...
			if (in_array($type, array('post', 'page', 'posttype'))) {
				...
				$__postType = $this->getPostType();
				if ( !empty($__postType) && $__postType == 'post' ) {
					$__post = $this->post;
				}
		...
		protected function get_taxonomy( $type, $obj) {
			...
			if (in_array($type, array('category', 'tag', 'taxonomy'))) {
				...
				if ($type=='category') {
					...
					if ( $__postType == 'term' ) {
						$__category = get_the_category();
						$__categ = array('name' => '', 'desc' => '');
			...
			if (in_array($type, array('post', 'page', 'posttype'))) {
				...
				if ( function_exists( 'get_the_terms' ) ) { //Since: 2.5.0 WP version
					...
					foreach ( $__taxonomies as $taxonomy_slug => $taxonomy ) {
						if (in_array($taxonomy_slug, array('category', 'post_tag', 'post_format'))) {
							continue 1;
						}
		...
		protected function getTaxonomyItems( $items, $first = false, $field = 'name') {
			...
			foreach ( $items as $k=>$v ) {
				if ($field=='name') {
					...
					$value = $v->name; //default return name!
				}
...
//$psp_TitleMetaFormat = new psp_TitleMetaFormat($this->cfg, ( isset($module) ? $module : array()) );
$psp_TitleMetaFormat = psp_TitleMetaFormat::getInstance();
