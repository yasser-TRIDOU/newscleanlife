...
if (class_exists('psp_aaModulesManger') != true) {
	class psp_aaModulesManger {
		...
		public function printListInterface() {
			...
			<div id="psp-main-loading">
				...
				<div id="psp-loading-box">
					<div class="psp-loading-text">' .esc_html__('Loading', 'smartSEO') . '</div>
			...
			foreach ($this->cfg['modules'] as $key => $value) {
				...
				if ($value['status'] == true) {
					if (!in_array($key, $this->cfg['core-modules'])) {
						...
						$html[] = '<i>' .esc_html__('Core Modules, can\'t be deactivated!', 'smartSEO') . '</i>';
					}
				} else {
					$html[] = '<a href="#activate" class="psp_action_button psp_activate" rel="' . ( $key ) . '">' .esc_html__('Activate', 'smartSEO') . '</a>';
				}
				
				$html[] = 	'<a href="#" class="psp_read_more">' . (esc_html__('read more', 'smartSEO') ) . '</a>';
