...
if (class_exists('psp_ServerStatusAjax') != true) {
	class psp_ServerStatusAjax extends psp_ServerStatus {
		...
		*/
		public function ajax_request() {
			$return = array();
			$actions = isset($_REQUEST['sub_action']) ? explode(',', sanitize_text_field($_REQUEST['sub_action'])) : '';
			...
			if ( in_array( 'check_memory_limit', array_values($actions)) ) {
				...
				$html = array();
				if ( $memory < 127108864 ) {
					$html[] = '<div class="psp-message psp-error">' . sprintf( esc_html__('%1$s - We recommend setting memory to at least 128MB. See: <a href="%2$s">Increasing memory allocated to PHP</a>', 'smartSEO' ), size_format( $memory ), 'http://codex.wordpress.org/Editing_wp-config.php#Increasing_memory_allocated_to_PHP' ) . '</div>';
				} else {
			...
			if ( in_array( 'export_log', array_values($actions)) ) {
				...
				$log = isset($_REQUEST['log']) ? sanitize_text_field($_REQUEST['log']) : '';
				$temp_file = tmpfile();
			...
			if ( in_array( 'remote_get', array_values($actions)) ) {
				...
				if ( ! is_wp_error( $response ) && $response['response']['code'] >= 200 && $response['response']['code'] < 300 ) {
					$msg =esc_html__('wp_remote_get() was successful', 'smartSEO' );
					$status = true;
				} elseif ( is_wp_error( $response ) ) {
					$msg = esc_html__('wp_remote_get() failed. Webservices Amazon won\'t work with your server. Contact your hosting provider. Error:', 'smartSEO' ) . ' ' . $response->get_error_message();
					$status = false;
				} else {
					$msg = esc_html__('wp_remote_get() failed. Webservices Amazon may not work with your server.', 'smartSEO' );
					$status = false;
				}
			...
			if ( in_array( 'check_soap', array_values($actions)) ) {
				...
				if ( class_exists( 'SoapClient' ) ) {
					$msg =esc_html__('Your server has the SOAP Client class enabled.', 'smartSEO' );
					$status = true;
				} else {
					$msg = sprintf( esc_html__('Your server does not have the <a href="%s">SOAP Client</a> class enabled - some gateway plugins which use SOAP may not work as expected.', 'smartSEO' ), 'http://php.net/manual/en/class.soapclient.php' ) . '</mark>';
					$status = false;
				}
					...
					if ( ! empty( $plugin_data['Name'] ) ) {
						...
						if ( strstr( $dirname, 'smartSEO' ) ) {
							...
							if ( ! empty( $version_data['version'] ) && version_compare( $version_data['version'], $plugin_data['Version'], '!=' ) ) {
								$version_string = ' &ndash; <strong style="color:red;">' . $version_data['version'] . ' ' . esc_html__('is available', 'smartSEO' ) . '</strong>';
							}
						}

						$wc_plugins[] = $plugin_data['Name'] . ' ' . esc_html__('by', 'smartSEO' ) . ' ' . $plugin_data['Author'] . ' ' . esc_html__('version', 'smartSEO' ) . ' ' . $plugin_data['Version'] . $version_string;
			...
			if ( in_array( 'smushit_remote_get', array_values($actions)) ) {
				...
				if ( ! is_wp_error( $response ) && $response['response']['code'] >= 200 && $response['response']['code'] < 300 ) {
					$msg =esc_html__('Connection to smushit.com server was successful', 'smartSEO' );
					$status = true;
				} elseif ( is_wp_error( $response ) ) {
					$msg = esc_html__('Connection to smushit.com server failed. Contact your hosting provider. Error:', 'smartSEO' ) . ' ' . $response->get_error_message();
					$status = false;
				} else {
					$msg = esc_html__('Connection to smushit.com server failed.', 'smartSEO' );
					$status = false;
				}
			...
			if ( in_array( 'tiny_compress_remote_get', array_values($actions)) ) {
				...
				if ( ! is_wp_error( $response ) && $response['response']['code'] >= 200 && $response['response']['code'] < 300 ) {
					$msg = sprintf(esc_html__('Connection to %s server was successful', 'smartSEO' ), TINYCOMPRESS_SERVER );
					$status = true;
				} elseif ( is_wp_error( $response ) ) {
					$msg = sprintf( esc_html__('Connection to %s server failed. Contact your hosting provider. Error:', 'smartSEO' ), TINYCOMPRESS_SERVER ) . ' ' . $response->get_error_message();
					$status = false;
				} else {
					$msg = sprintf( esc_html__('Connection to %s server failed. Error:', 'smartSEO' ), TINYCOMPRESS_SERVER ) . ' ' . $response['response']['code'] . ' - ' . $response['response']['message'];
					$status = false;
				}
		...
		public function verify_step() {
			$module = isset($_REQUEST['module']) ? trim(sanitize_text_field($_REQUEST['module'])) : '';
			$action = isset($_REQUEST['sub_action']) ? trim(sanitize_text_field($_REQUEST['sub_action'])) : '';
			...
			if ( $module == 'google_analytics' ) {
				...
				if ( $action == 'step1' ) {
					...
					$return = array(
						...
						'log' 		=> $mandatoryValid ?esc_html__('all mandatory fields: client id, client secret, redirect uri are set!', 'smartSEO') :esc_html__('some of the mandatory fields: client id, client secret, redirect uri are NOT set!', 'smartSEO'),
			...
			if ( $module == 'serp' ) {
				...
				if ( $action == 'step1' ) {
					...
					$return = array(
						'status'	=> $mandatoryValid ? 'valid' : 'invalid',
						'log' 		=> $mandatoryValid ?esc_html__('all mandatory fields: developer key, custom search id, google country are set!', 'smartSEO') :esc_html__('some of the mandatory fields: developer key, custom search id, google country are NOT set!', 'smartSEO'),
						'execution_time' => number_format( microtime(true) - $start, 2)
					);
				} // end step 1
			...
			if ( $module == 'pagespeed' ) {
				...
				if ( $action == 'step1' ) {
					...
					$return = array(
						'status'	=> $mandatoryValid ? 'valid' : 'invalid',
						'log' 		=> $mandatoryValid ?esc_html__('all mandatory fields: developer key, google language are set!', 'smartSEO') :esc_html__('some of the mandatory fields: developer key, google language are NOT set!', 'smartSEO'),
						'execution_time' => number_format( microtime(true) - $start, 2)
					);
				} // end step 1
				...
				if ( $action == 'step2' ) {
					...
					$_REQUEST['link'] = get_permalink( (int) sanitize_text_field($_REQUEST['postid']) );
					$getGraph = $pspPagespeed->check_page( sanitize_text_field($_REQUEST['link']), sanitize_text_field($_REQUEST['postid']) );
			...
			if ( $module == 'facebook_planner' ) {
				...
				if ( $action == 'step1' ) {
					...
					$return = array(
						'status'	=> $mandatoryValid ? 'valid' : 'invalid',
						'log' 		=> $mandatoryValid ?esc_html__('all mandatory fields: app id, app secret, language are set!', 'smartSEO') :esc_html__('some of the mandatory fields: app id, app secret, language are NOT set!', 'smartSEO'),
						'execution_time' => number_format( microtime(true) - $start, 2)
					);
				} // end step 1
			...
			if ( $module == 'tinycompress' ) {
				...
				if ( $action == 'step1' ) {
					...
					$return = array(
						'status'    => $mandatoryValid ? 'valid' : 'invalid',
						'log'       => $mandatoryValid ?esc_html__('all mandatory fields: tiny api key, image sizes are set!', 'smartSEO') :esc_html__('some of the mandatory fields: tiny api key, image sizes are NOT set!', 'smartSEO'),
						'execution_time' => number_format( microtime(true) - $start, 2)
					);
				} // end step 1
